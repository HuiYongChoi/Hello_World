...
**빠른 조건** 버튼이 자주 쓰는 조합을 한 번에 넣습니다. 매번 일곱 칸을 맞추게
하면 조건을 바꿔 보는 일 자체를 안 하게 됩니다. 첫 번째가 이 화면을 만든
이유입니다 — `함안·의령 출퇴근 · 방2↑ · 15평↑` (마산회원구 · 전용 45㎡ 이상 ·
출퇴근 가까운 순).

정렬에 **함안·의령 가까운 순**이 있습니다. 등급이 같으면 싼 순입니다 —
출퇴근만으로 줄을 세우면 같은 등급 안에서 순서가 임의가 되어 "왜 이게 위지"
가 됩니다.

찾기와 대출은 **서로를 바꿉니다.** 후보 카드의 "이 집으로 대출 계산" 이 그 집
보증금을 표로 밀어 넣고, 반대로 표의 "금리를 기회비용률로 가져오기" 가 목록의
월 환산 주거비를 다시 계산합니다 — 보증금을 대출로 채우면 그 이자가 곧
기회비용이기 때문입니다.
