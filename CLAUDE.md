...
| 집값 상승률 | 실측 앵커 | `growth.ts` — 화면에서 가져다 쓰는 방식. **연쇄 드리프트로 위로 치우칩니다** (아래) |
...
### 연쇄 지수는 위로 치우칩니다 — 아직 안 고친 것

`growth.ts` 의 연쇄 지수는 분기마다 같은 칸의 가격비 **중위를 곱해** 이어 붙입니다.
구성 변화는 막아 주지만, 곱셈을 반복하는 동안 위쪽으로 치우칩니다.
...
모든 진입분기에서 지수가 2~4%p 위에 있습니다. 즉 화면이 "가져다 쓰세요" 로
제안하는 지역 CAGR(창원 1.73 · 부산 4.42 · 경기 5.57)도 **전형적인 한 칸이 겪은
것보다 높을 가능성**이 있습니다. 백테스트는 코호트 기준선으로 이 문제를 피했지만
`growth.ts` 자체는 아직 그대로입니다 — 바꾸면 3-way 결론이 같이 움직이므로
별도 판단이 필요합니다.
